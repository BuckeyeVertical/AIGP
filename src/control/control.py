...
def main():
    ...
    phase = "takeoff"
    dash_until = 0.0
    last_good = None  # (t, max bbox px, ex) of the last confident detection
    for item in frames(args.cam_ip, args.cam_port, timeout_s=2.0):
        ...
        if phase == "servo":
            ...
            if good:
                ...
                if size >= BLIND_MIN_SIZE_PX and abs(ex) <= BLIND_MAX_EX:
                    last_good = (now, size, ex)

            blind_pass = (
                not good
                and last_good is not None
                and now - last_good[0] <= BLIND_RECENT_S
            )
            if (
                good
                and max(det.width_px, det.height_px) >= COMMIT_SIZE_PX
                and abs(ex) <= COMMIT_MAX_EX
            ):
                print(
                ...
                cmd = servo.compute(
                    det if det is not None else _NO_GATE,
                    reported_pitch=telem.pitch,
                )
